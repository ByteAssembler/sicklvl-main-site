...
export async function getPortfolioWithInfos(slug: string) {
    ...
    const circularPreviousPortfolio =
        previousPortfolio ||
        (await prismaClient.portfolioItem.findFirst({
            orderBy: {
                order: "desc",
            },
            include: {
    ...
    if (!circularPreviousPortfolio || !circularNextPortfolio) {
        throw new Error(
            "Portfolio count is not enough for circular navigation",
        );
    }
