...
export const GET: APIRoute = async ({ request, params, locals, cookies, url }) => {
	const { slug } = params;
	if (!slug || typeof slug !== "string") return errorConditionerHtmlResponse("Not found");
	...
	if (!boxFile.mime) return errorConditionerHtmlResponse("Mime type not found");

	if (BOXhasAccessToFile(
		boxFile,
		locals.admin,
		locals.customer?.id,
		...
		return errorConditionerHtmlResponse('You do not have access to this box');
	}
	...
	if (range) {
		...
		return new Response(webReadableStream, {
			...
			headers: {
				...
				'Content-Disposition': download ? `attachment; filename="${boxFile.file_name}"` : `inline; filename="${boxFile.file_name}"`,
