...
export async function GET(context: APIContext): Promise<Response> {
	try {
		...
		if (!box || !box.box_files.length) {
			return errorConditionerHtmlResponse('Box not found or no files available');
		}

		if (BOXhasAccessToFolder(
			box,
					...
					archive.on('data', (chunk) => {
						if (!isCanceled) {
							try {
								...
								console.warn('Error during enqueue, possibly due to stream being closed.', err);
								isCanceled = true;
		...
		return errorConditionerHtmlResponse('An error occurred while creating the ZIP file');
	}
}
